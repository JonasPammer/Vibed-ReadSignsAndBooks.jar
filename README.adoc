...
** Separated by `#region` and `#endregion` markers for VSCode folding
...
** Load with https://github.com/maruohon/litematica[Litematica mod]

* `books_commands.litematic` - Litematica schematic with command blocks for book distribution
** Chain of command blocks that `/give` shulker boxes organized by author
...
=== Using Litematica Schematics

The tool generates Litematica `.litematic` files that can be loaded with the https://github.com/maruohon/litematica[Litematica mod]:

. *Install Litematica* for your Minecraft version (requires https://fabricmc.net/[Fabric] or https://github.com/FabricMC/fabric-loader[similar loader])

. *Load the schematic:*
** Open Litematica menu (default: `M` key)
** Go to "Load Schematics"
** Navigate to your `ReadBooks/YYYY-MM-DD/` folder
** Select `signs.litematic` or `books_commands.litematic`
